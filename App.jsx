...
const PHASES = [
  ...
  },
  {
    ...
    status: "active",
    tag: "In progress",
    detail: "Candidates file nomination papers with the Returning Officer of their constituency. Each Lok Sabha candidate must pay a security deposit of Rs.25,000 (Rs.12,500 for SC/ST candidates). Nominations go through scrutiny; invalid nominations are rejected. Candidates can withdraw within 2 days of scrutiny.",
  ...
  },
  {
    ...
    status: "upcoming",
    tag: "Upcoming",
    detail: "Parties and candidates campaign across their constituencies through rallies, media, and door-to-door outreach. Campaign spending is capped at Rs.95 lakhs per Lok Sabha candidate. The silence period begins 48 hours before polling -- no campaigning, rallies, or exit poll broadcasts are permitted.",
...
const QUIZ_QUESTIONS = [
  ...
  },
  {
    ...
    opts: ["Rs.10,000", "Rs.25,000", "Rs.50,000", "Rs.1,00,000"],
    ans: 1,
    explanation: "Lok Sabha candidates must deposit Rs.25,000 (Rs.12,500 for SC/ST). It is forfeited if they receive less than 1/6 of the votes polled.",
  },
];
  ...
  return (
    <div style={{ display: "flex", gap: 12, marginBottom: 4 }}>
      <div style={{ display: "flex", flexDirection: "column", alignItems: "center" }}>
        <div
          style={{ width: 32, height: 32, borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 13, backgroundColor: c.bg, border: "1.5px solid " + c.border, color: c.dot, fontWeight: 700, flexShrink: 0 }}
          aria-hidden="true"
        >
          {phase.status === "done" ? "v" : phase.id}
        </div>
      ...
      <div style={{ flex: 1, paddingBottom: 16 }}>
        ...
        >
          <div>
            <span style={{ fontSize: 15, fontWeight: 600, color: "#1a1a1a" }}>{phase.title}</span>
            <div style={{ marginTop: 3 }}>
              <span style={{ fontSize: 11, padding: "2px 8px", borderRadius: 10, backgroundColor: c.bg, color: c.tag, fontWeight: 500 }}>
                {phase.tag}
              </span>
            </div>
          </div>
          <div style={{ textAlign: "right", flexShrink: 0 }}>
            ...
            <div style={{ fontSize: 11, color: "#1565C0", marginTop: 4 }}>{isOpen ? "Hide" : "Show details"}</div>
          </div>
        </button>
...
function Quiz() {
  ...
  const handleAnswer = useCallback((i) => {
    ...
    if (announcerRef.current) {
      announcerRef.current.textContent = i === current.ans ? "Correct! " + current.explanation : "Incorrect. " + current.explanation;
    }
  }, [chosen, current]);
      ...
      <div style={{ maxWidth: 960, margin: "0 auto", background: "#fff", minHeight: "100vh", boxShadow: "0 0 0 1px #E0E0E0" }}>
        <header style={{ padding: "1rem 1.5rem", borderBottom: "1px solid #E0E0E0", display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 8 }} role="banner">
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <div style={{ width: 36, height: 36, borderRadius: "50%", background: "#1565C0", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 16, color: "#fff", fontWeight: 700 }} aria-hidden="true">EQ</div>
            <div>
